...
const TOAST_SCENE_PATH: String = "res://ui/components/AppToast.tscn"
...
static func show_toast(text: String, toast_type: Type = Type.INFO) -> void:
	...
	if toast_scene == null:
		return
	var toast: AppToast = toast_scene.instantiate() as AppToast
...
func _apply_type_color(toast_type: Type) -> void:
	var text_color: Color = Color("f5efe7")
	match toast_type:
		Type.INFO:
			text_color = Color("f5efe7")
		Type.SUCCESS:
			text_color = Color("86efac")
		Type.WARNING:
			text_color = Color("fde68a")
		Type.ERROR:
			text_color = Color("fca5a5")
